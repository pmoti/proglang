fun is_older(earlierDt : int*int*int, laterDt : int*int*int) = 
     if (#1 earlierDt = #1 laterDt) 
     then
	if (#2 earlierDt = #2 laterDt) 
	then #3 earlierDt < #3 laterDt
	else #2 earlierDt < #2 laterDt
    else #1 earlierDt < #1 laterDt
...
fun reasonable_date(date : int*int*int) = 
	...
	fun exists(list_to_check : int list, value : int) =
	    if (hd list_to_check = value) 
	    then true
	    else 
		if null (tl list_to_check) 
		then false
